...
class CommandOrchestrator:
    ...
    def __init__(
        ...
        self._update_bet_status_use_case = (
    ...
    async def execute_update_bet_command(
        self, bet_id: str, new_status: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"🔄 Ejecutando comando UPDATE_BET (bet_id={bet_id}, status={new_status})"
        )
        ...
        try:
            ...
            if new_status not in valid_statuses:
                return {"success": False, "error": f"Estado inválido: {new_status}"}

            # Ejecutar use case
            success = await self._update_bet_status_use_case.execute(
                bet_id=bet_id, new_status=status_enum
            )
    ...
    async def execute_list_bets_command(
        ...
        """
        logger.info(
            f"📋 Ejecutando comando LIST_BETS (status={status_filter}, limit={limit})"
        )
    ...
    def _calculate_statistics(self, bets: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        if not bets:
            return {"total": 0, "pending": 0, "won": 0, "lost": 0, "void": 0}

        stats = {"total": len(bets), "pending": 0, "won": 0, "lost": 0, "void": 0}
        ...
        for bet in bets:
            ...
            if status:
                ...
                if "pendiente" in status_lower or "pending" in status_lower:
                    ...
                    "ganada" in status_lower
                    or "won" in status_lower
                    or "ganado" in status_lower
                ):
                    stats["won"] += 1
                elif (
                    "perdida" in status_lower
                    or "lost" in status_lower
                    or "perdido" in status_lower
                ):
                    stats["lost"] += 1
