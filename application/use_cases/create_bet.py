    ...
    async def execute(self, dto: CreateBetDTO) -> BetDTO:
        ...
        if dto.image_filename and dto.image_file_path:
            image = BetImage.from_telegram_file(
                filename=dto.image_filename, file_path=dto.image_file_path
            )
    ...
    def _to_dto(self, bet: Bet) -> BetDTO:
        ...
        return BetDTO(
            ...
            potential_profit_amount=(
                float(bet.potential_profit.amount) if bet.potential_profit else None
            ),
            potential_profit_currency=(
                bet.potential_profit.currency if bet.potential_profit else None
            ),
