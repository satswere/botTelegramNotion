...
class StatusHandler:
    ...
    def __init__(
        ...
        logger.info(
            "📊 StatusHandler inicializado (refactorizado con CommandOrchestrator)"
        )
    ...
    async def handle(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        logger.info(
            f"📊 Comando /status recibido de usuario {update.message.from_user.id}"
        )
    ...
    async def _check_notion_connection(self) -> str:
        ...
        try:
            _ = self._notion_client.databases.retrieve(self._database_id)
            return "✅ Conectado"
        except Exception as e:
