...
def analyzer(mock_openai_handler):
    ...
    with patch(
        "infrastructure.openai.openai_image_analyzer.OpenAIHandler",
        return_value=mock_openai_handler,
    ):
...
class TestOpenAIImageAnalyzerAnalyzeImage:
    ...
    async def test_analyze_image_success(self, analyzer, mock_openai_handler):
        ...
        # Arrange
        mock_openai_handler.analyze_image.return_value = (
            "Image contains a sports bet slip"
        )

        # Act
        result = await analyzer.analyze_image(
            image_path="test.jpg", prompt="Analyze this image"
        )
    ...
    async def test_analyze_image_file_not_found(self, analyzer, mock_openai_handler):
        ...
        # Arrange
        mock_openai_handler.analyze_image.side_effect = FileNotFoundError(
            "File not found"
        )
    ...
    @pytest.mark.asyncio
    async def test_analyze_image_malformed_response(
        self, analyzer, mock_openai_handler
    ):
...
class TestOpenAIImageAnalyzerExtractBetInfo:
    ...
    @pytest.mark.asyncio
    async def test_extract_bet_info_handles_null_values(
        self, analyzer, mock_openai_handler
    ):
