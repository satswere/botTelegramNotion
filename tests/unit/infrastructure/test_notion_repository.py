...
def repository(mock_notion_client):
    ...
    repo = NotionBetRepository(
        notion_client=mock_notion_client, database_id="mock-database-id"
    )
    return repo
...
class TestNotionBetRepositorySave:
    ...
    @pytest.mark.asyncio
    async def test_save_handles_api_response_error(
        self, repository, mock_notion_client
    ):
...
class TestNotionBetRepositoryFindById:
    ...
    async def test_find_by_id_success(self, repository, mock_notion_client):
        ...
        mock_notion_client.pages.retrieve.return_value = {
            "id": "page-123",
            "properties": {"Evento": {"title": [{"text": {"content": "Test Event"}}]}},
        }
    ...
    async def test_find_by_id_not_found(self, repository, mock_notion_client):
        ...
        # Arrange
        api_error = APIResponseError(
            response=Mock(status=404), message="Page not found", code="object_not_found"
        )
...
class TestNotionBetRepositoryUpdateStatus:
    ...
    @pytest.mark.asyncio
    async def test_update_status_handles_api_error(
        self, repository, mock_notion_client
    ):
        """Test manejo de error al actualizar estado"""
        # Arrange
        api_error = APIResponseError(
            response=Mock(status=400), message="Invalid status", code="validation_error"
        )
