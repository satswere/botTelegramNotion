...
class TelegramMessageExtractor:
    ...
    def extract_metadata(self, message: Message) -> Dict[str, Any]:
        ...
        """
        logger.debug(
            f"📧 Extrayendo metadatos del mensaje ID: {getattr(message, 'message_id', 'unknown')}"
        )
        ...
        try:
            ...
            logger.info(
                f"✅ Metadatos extraídos exitosamente - Chat: {message_data['chat']['chat_id']}"
            )
            return message_data
            ...
            logger.error(
                f"❌ Error inesperado extrayendo metadatos: {e}", exc_info=True
            )
    ...
    def extract_forward_info(self, message: Message) -> Optional[Dict[str, Any]]:
        ...
        if forward_origin:
            if hasattr(forward_origin, "sender_user") and forward_origin.sender_user:
                ...
                origin_info["origin_sender_name"] = (
                    f"{getattr(sender_user, 'first_name', '') or ''} {getattr(sender_user, 'last_name', '') or ''}".strip()
                )
                origin_info["origin_sender_username"] = getattr(
                    sender_user, "username", None
                )

            elif (
                hasattr(forward_origin, "sender_user_name")
                and forward_origin.sender_user_name
            ):
                origin_info["origin_sender_name"] = getattr(
        ...
        is_forwarded = bool(
            ...
            or forward_sender_name
            or forward_date
            or forward_origin
            or is_automatic_forward
        )

        if not is_forwarded:
            return {"is_forwarded": False, "origin_info": {}}
    ...
    async def extract_file(self, message: Message) -> Optional[Dict[str, Any]]:
        ...
        """
        logger.debug(
            f"📎 Extrayendo archivo del mensaje ID: {getattr(message, 'message_id', 'unknown')}"
        )
        ...
        try:
            if message.photo:
                ...
                logger.info(
                    f"✅ Foto extraída: {file_info['file_id'][:20]}... ({file_info['file_size']} bytes)"
                )
                return file_info
                ...
                logger.info(
                    f"✅ Documento extraído: {file_info.get('file_name', 'sin_nombre')}"
                )
                return file_info
    ...
    def _generate_unique_identifier(
        self, origin_info: Dict, forward_date
    ) -> Optional[str]:
        ...
        # Fecha
        date_str = origin_info.get("origin_date") or (
            forward_date.isoformat() if forward_date else None
        )
        if date_str:
