...
class LocalFileStorage:
    ...
    async def save(
        ...
        try:
            ...
            raise FileStorageError(f"Error de disco guardando {filename}: {str(e)}") from e
            
        except Exception as e:
            logger.error(f"❌ Error inesperado guardando archivo {filename}: {e}", exc_info=True)
    ...
    async def load(self, filename: str) -> bytes:
        ...
        try:
            ...
            logger.info(f"✅ Archivo cargado exitosamente: {filename} ({len(data)} bytes)")
            return data
            ...
            raise FileStorageError(f"Error de disco cargando {filename}: {str(e)}") from e
            
        except Exception as e:
            logger.error(f"❌ Error inesperado cargando archivo {filename}: {e}", exc_info=True)
    ...
    async def get_size(self, filename: str) -> int:
        ...
        try:
            ...
            logger.error(f"❌ Error inesperado obteniendo tamaño de {filename}: {e}", exc_info=True)
